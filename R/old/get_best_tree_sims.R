...
#' @param save.plot a logical indicating whether plots of both the true and
#' best tree should be saved. If \code{TRUE}, must specify \code{project} and
#' \code{outpath}.
...
#'
#' @examples
get_best_tree_sims<-function(truetree, get.trees.out, Ktrue,
                             criterion=c("AIC","BIC","DIC"), save.muts=F,
                             save.plot=F, project=NULL, outpath=NULL){

  # check arguments
  if (!inherits(truetree, "phylo")){
    stop("truetree must be of class \"phylo\"")
  }
  ...
  if (Ktrue <= 0 | !is.numeric(Ktrue) | Ktrue!=round(Ktrue)){
    stop("Ktrue must be an integer greater than 0")
  }
  ...
  # Plot true phylogeny and output mutations at each node
  true.mut.list<-plot_tree_test(tree=truetree, Rs, N, S,
                                ptitle=paste0("True Tree: K=",Ktrue),
                                annovar=NULL, save.muts=save.muts, save.plot=save.plot,
                                outpath=outpath, project=project)
  graphics::plot.new()
